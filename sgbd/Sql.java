...
public class Sql {
    ...
    public int[] issSubRequest(String req) {
        int[] ans = new int[2];
        char[] chr = req.toCharArray();
    ...
    public Vector<Table> SELECT(String rqt) throws Exception {
        ...
        if(sub[0] != -1) {
            myRQT = rqt.split(" ", 4);
            String subReq = rqt.substring(sub[0]+1, sub[1]);

            Vector<Table> subR = SELECT(subReq);
            String[] show = Colonne(subR.get(0).getAllNom(), myRQT[1]);

            return Projection(subR, show);
        }
        ...
        if(myRQT.length == 4) {
            ...
            } else if(myRQT[4].equalsIgnoreCase("DIVISE")) {
                System.out.println("\n-> " + rqt + " avec division\n");
                System.out.println("\n'" + myRQT[3] + "' divise par '" + myRQT[5].split(" ")[1] + "'\n");

                Vector<Table> R = Division(file.getTableContent(myRQT[3]), file.getTableContent(myRQT[5].split(" ")[1]));
                show = Colonne(R.get(0).getAllNom(), myRQT[1]);
